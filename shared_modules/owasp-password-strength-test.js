 export let owaspConfig = {};

 let isNode = false;
 let configSet = false;

 // Since this is a shared module, we need to determine if the process is running in Node or browser
 // Based on this, additional imports will need to be run to set owasp config
 if ((typeof process !== 'undefined') &&
   (process.release.name.search(/node/) !== -1)) {
   isNode = true;
 }
 ...
 async function init() {
   if (isNode && !configSet) {
     ...
     let configPath = url.pathToFileURL(path.resolve('./config/config.js')).href;
     // eslint-disable-next-line node/no-unsupported-features/es-syntax
     await import(configPath).then(async (config) => {
       config = await config.default;
       owaspConfig = config && config.owaspConfig ? config.owaspConfig : {};
       configSet = true;
     });
   }
 }
