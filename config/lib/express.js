 ...
 */
var initServerModels = function (app, config) {
 ...
 */
var initHandlebars = function (app, config) {
 ...
 */
var initHelmetHeaders = function (app, config) {
...
var enableCORS = function (app, config) {
    app.use(function (req, res, next) {
...
var init = function (config, db) {
    ...
    initHandlebars(app, config);

    // Initialize Express view engine
    initViewEngine(app, config);

    // Initialize Helmet security headers
    initHelmetHeaders(app, config);
    ...
    console.log('Loading Completed for: ' + config.appName);
    return app;
};
...
var initApps = function (db) {
    ...
    // intialize each module
    let menuConfig = [];
    let allConfigs = [];
    config.submodules.forEach(module => {
        var originalConfig = _.cloneDeep(config);
        var appConfig = require(path.resolve(module.appConfig));
        var newConfig = _.merge(originalConfig, appConfig);

        var moduleEnvConfig = require(path.join(process.cwd(), module.basePath + '/env', process.env.NODE_ENV)) || {};
        // Requiring module's config.js will have all configs from app level
        appConfig = _.merge(appConfig, newConfig, moduleEnvConfig);

        // assigning services to configs.
        appConfig.services = require(path.resolve('./app/services.js'));

        // assigning helpers to configs
        appConfig.helpers = require(path.resolve('./app/helpers.js'));

        // prepend module routes to core routes
        Array.prototype.unshift.apply(appConfig.files.server.routes, module.routes);

        // prepend module configurations to core configurations
        Array.prototype.unshift.apply(appConfig.files.server.configs, module.config);

        // set up view
        appConfig.appViewPath = module.basePath + '/server';

        // set up static file location
        appConfig.staticFiles = 'dist/' + path.basename(module.basePath) + '/';

        // set up config for client. emited view /get method
        appConfig[appConfig.appName] = appConfig.APP_CLIENT_CONFIG;

        // add app to vhost
        if (appConfig.MENU_CONFIG && appConfig.MENU_CONFIG.length) {
            appConfig.MENU_CONFIG.forEach(menu => {
                menu.appBase = appConfig.appBase;
            });
            menuConfig = [...menuConfig, ...appConfig.MENU_CONFIG];
        }

        allConfigs.push(appConfig);
    });

    allConfigs.forEach(function (appConfig) {
        appConfig.MENU_CONFIG = menuConfig;
        if (appConfig.appBase) {
            rootApp.use(appConfig.appBase, init(appConfig, db));
        } else if (appConfig.domainPattern) {
            rootApp.use(vhost(appConfig.domainPattern, init(appConfig, db)));
        }
    });
    ...
    rootApp.use(function (req, res, next) {
        if (req.originalUrl.length <= 1) {
            ...
            if (!allConfigs.find(conf => conf.appBase === toApp)) {
                toApp = '/';
            }
            res.redirect(toApp);
        } else if (req.originalUrl === '/favicon.ico') {
            res.sendFile('src/css/icons/favicon.ico', {
                root: '.'
            });
        } else {
            next();
