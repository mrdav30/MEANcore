...
export class AuthService {
    ...
    public APP_BASE = environment.APP_BASE;
    public API_END_POINT = environment.API_END_POINT;
    public redirectUrl = environment.APP_DEFAULT_ROUTE;
    public user: any = false;
    public userChange$: Observable<any>;

    private signInUrl = this.APP_BASE + this.API_END_POINT + 'auth/signIn'; // URL to web API
    private recoverUrl = this.APP_BASE + this.API_END_POINT + 'auth/forgot';
    private resetUrl = this.APP_BASE + this.API_END_POINT + 'auth/reset/';
    private signOutUrl = this.APP_BASE + this.API_END_POINT + 'auth/signOut';
    private validateUrl = this.APP_BASE + this.API_END_POINT + 'auth/validate';
    private signUpUrl = this.APP_BASE + this.API_END_POINT + 'auth/signUp';
    private installUrl = this.APP_BASE + this.API_END_POINT + 'api/install';
    ...
    constructor(
        ...
        if (this.configService.user && this.configService.user._id) {
    ...
    hasPermission(permission): boolean {
        ...
        const permissions = this.user.permission_data[this.APP_NAME];
        return !!_.find(permissions, (p) => p.name === permission);
    }

    checkPermission(permission, attributes): any {
        permission = permission.toLowerCase();
        const permissions = _.filter(this.user.permission_data[this.APP_NAME], (p) => p.name === permission);
